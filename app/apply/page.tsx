  ...
  async function submit() {
    ...
    try {
      ...
      if (result.success) {
        ...
        setName(''); setEmail(''); setMemberNumber(''); setChildNumbers(''); setPhone('');
      } else {
  ...
  return (
    <div className="stack">
      ...
      <section className="card stack">
        <div className="row">
          <button className={`btn ${type === 'parent' ? 'primary' : ''}`} onClick={() => { setType('parent'); setChildNumbers(''); }}>👨‍👩‍👧 家長</button>
        ...
        {type !== 'member' && (
          <label>Email *（審批後用作登入）<input type="email" value={email} onChange={e => setEmail(e.target.value)} placeholder="your@email.com"/></label>
        )}
        ...
        {type === 'member' && (
          <label>YMIS / 成員編號 *
            <input value={memberNumber} onChange={e => setMemberNumber(e.target.value)} placeholder="例如：1234567890 或 2501"/>
          </label>
        )}
