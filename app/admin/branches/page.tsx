...
function branchHint(id:string){ if(id==='b1') return '小童軍預設沒有分隊。'; if(id==='b2') return '幼童軍按顏色分隊 / 六。'; if(id==='b3') return '童軍按動物名稱小隊。'; return '此支部預設沒有分隊，如需要可自行新增。'; }
export default function Page(){
  ...
  return <div className="stack"><section className="hero"><span className="badge gold">支部管理</span><h1>支部與小隊 / 六設定</h1><p>新增及停用小隊 / 六。</p></section>
    ...
    <section className="grid-wide"><div className="card stack"><h2>{branches.find(b=>b.id===selected)?.name} · 小隊 / 六設定</h2><p className="muted">{branchHint(selected)}</p>
      ...
      <div className="grid"><input value={name} onChange={e=>setName(e.target.value)} placeholder="新增名稱，例如 紅色六 / 猛虎小隊"/><input value={short} onChange={e=>setShort(e.target.value)} placeholder="簡稱"/></div>
      <button className="btn primary" onClick={add}>新增小隊 / 六</button></div>
