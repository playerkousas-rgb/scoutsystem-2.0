...
export default function TopNav(){
  ...
  useEffect(()=>{setSess(getSession())},[pathname]);
  ...
  return <header className="topbar"><div className="topbar-inner">
    ...
    <nav className="nav">
      ...
      <Link className="btn" href="/activities">活動</Link>
      <Link className="btn" href="/library">圖書館</Link>
      {!session && <><Link className="btn optional" href="/setup">接入</Link><Link className="btn optional" href="/downloads">下載</Link><Link className="btn optional" href="/updates">更新</Link><Link className="btn optional" href="/troops">使用旅團</Link></>}
      ...
      {session ? <><Link className="btn primary" href={dash}>控制台</Link><span className="badge blue">{session.name}</span><button className="btn" onClick={logout}>登出</button></> : <Link className="btn primary" href="/login">登入</Link>}
    </nav>
  </div></header>
}
